...
namespace ComandSenderManager
{
    public class ComandSender : IComandSender
    {
        ...
        public async Task<Session> CreateSession(string opcIp)
        {
            ...
            {
                _config.CertificateValidator.CertificateValidation += (s, e) => { e.Accept = (e.Error.StatusCode == Opc.Ua.StatusCodes.BadCertificateUntrusted); };
            }
        ...
        public async Task<string> CheckRemoteControlStatus(Session session, string hRemote)
        {
            var nodeValue = await session.ReadValueAsync(hRemote);
            return nodeValue.Value.ToString();
        }
    }
}
